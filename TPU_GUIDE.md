...
**Contact:** For any questions regarding the setup or usage of the TPU, please contact [Taiming Lu](mailto:tl0463@princeton.edu) via email or Slack.

The content of this guide was composed by [Boya Zeng](https://boyazeng.github.io), [Yufeng Xu](https://zephyr271828.github.io), and [Taiming Lu](https://taiminglu.com).

> **WARNING: Cost control is critical.** Unlike GPU clusters, TPU usage incurs real cloud costs (storage, networking, compute) that accumulate daily. Careless operations (e.g., cross-region transfers, uncompressed datasets, forgotten VMs) can result in significant charges. **Read the [Cost](#cost) section before starting any work**, and follow the [Job Requirements](#job-requirements) for naming and resource management rules.


## Overview
This guide is for using the Google Cloud TPU in Zhuang's group at Princeton University.

You can access these [slides](https://docs.google.com/presentation/d/1AaRlwMk87DRO-bsk1o1tqVCVyZC6mI0SCK1FvMya3Hw/edit?usp=sharing) to see a general introduction to TPU, Google Cloud, and JAX.
...
> **Tip:** Click the outline icon (top-right of this page on GitHub) to navigate by section.
 ...
 **Important**: Check your project's TPU quotas before requesting resources to avoid quota exceedance.
...
To train using multiple workers,
...
- **Bucket**: Google Cloud Storage (GCS) containers where you store datasets, logs, and checkpoints. Note that you only have 100 GB disk space on each TPU VM (host), so it’s necessary to store your large files in a bucket first, then access them by mounting to your VM or by using [gsutil](https://cloud.google.com/storage/docs/gsutil).
...
The TPU versions we have access to are v2, v3, v4, v5e, and v6e.
...
Each Google Cloud project has several considerations that affect your TPU usage.
...
![TPU Quota Console](./images/quota.png)
...
**Key Differences:**
- **Spot resources** are the newer version of preemptible resources and generally preferred over preemptible.
...
gcloud compute tpus tpu-vm create example_tpu \
  ...
  --spot
```
To request an on-demand TPU-VM, remove the `--spot` flag.
...
The TPU-VM request command would fail when there are no available resources. To queue for the resource:
```
# start queueing
...
```
Both spot and on-demand resources can be queued.
...
**To use the TPU-VM with VS Code:**
...
Then, go to the `Permissions` page, and click on `Grant access`. In the `New principals` field, put in `projectOwner:name-of-project-b` and set role to `Storage Legacy Object Reader`.

### Durable block storage

Disk has two modes: single-writer mode and read-only mode.
...
In read-only mode, formatting a non-boot disk isn't possible. Thus, if you want to use the disk on a multi-host TPU, you should format the non-boot disk first on a single-host machine in single-writer mode. You don't have to redo the formatting when you attach the disk to your multi-host TPU.
...
**Step 1:** On a single-host machine
...
**Step 2:** On the multi-host machine that you're actually using for e.g. model training
...
6. Only save one checkpoint for each run, unless deliberate and necessary.
7. Periodically clean up your buckets, e.g., outdated checkpoints and data.
8. Keep all operations within one zone to prevent transfer costs.
...
![Operational Cost](./images/operational_cost.png)
...
According to the "Data transfer within Google Cloud" table under https://cloud.google.com/storage/pricing#price-tables, **as long as the bucket and the TPU-VM are in the same location**, the data transfer is free.

- Bucket -> bucket transfer
According to the "Data transfer within Google Cloud" table under https://cloud.google.com/storage/pricing#price-tables,<br>
...
This means that transferring 1 TB of data between buckets would cost at least $20. Thus, if a copy of the data is also available locally, it's advised to upload it from local instead of copying between buckets.
...
It falls roughly under the following categories:
...
- Temporary block storage (e.g., local SSD): offers the fastest performance among all block storage types, with the tradeoff that the stored data is lost if the VM stops, suspends, restarts, crashes, or fails.
...
## Tools
Yufeng's Jobman provides a good option for TPU job management: https://github.com/Zephyr271828/jobman

## Feedback and Contact

**Feedback:** If you find any errors, unclear instructions, or have suggestions for improving this guide, please let [Taiming Lu](mailto:tl0463@princeton.edu) know. Your feedback helps us make this guide better for everyone.
